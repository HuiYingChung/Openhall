...
1. **Upload and describe.** The artist supplies JPG, PNG, or WebP images, optional title/medium/year metadata, and a one-sentence curatorial brief.
2. **AI analyses and curates.** A vision model analyses each work and suggests an artwork title only when the artist left it blank. A language model chooses room count, grouping, wall assignments, within-room visitor order, exhibition title, labels, and narration.
...
| Interpretation | Manual labels or optional add-ons | AI-suggested artwork titles for blanks, plus an AI-written exhibition title, labels, and narration |
...
- **F3. AI analysis** — `meta-llama/llama-3-2-11b-vision-instruct` analyses style, palette, subject, mood, description, and a concise artwork-title suggestion into validated JSON. An artist-entered title always wins; the suggestion fills only a blank title and adds no extra request.
- **F4. AI curation** — `meta-llama/llama-3-3-70b-instruct` chooses room count, grouping, wall assignments, and narrative order. Validation requires the order to move through rooms monotonically.
...
- **F6. AI writing** — a plain-text exhibition title plus validated, editable wall labels and spoken narration. Editable artwork title/medium/year metadata is kept out of writing and curation prompts, so review changes are deterministic and free. Medium is never AI-inferred and is omitted from `gallery.json` and public cards when blank. Structured failures retry once, then surface visibly.
...
```mermaid
flowchart TD
    U["Upload images + optional metadata"] --> A["Vision analysis + artwork title suggestions"]
    B["One-sentence brief"] --> C["Curation plan"]
    A --> C
    P["Style preset"] --> D["Deterministic rooms, placements, doorways, tour"]
    C --> D
    C --> W["Exhibition title, labels, narration"]
    A --> W
