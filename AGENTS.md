...
8. **Artwork title/medium/year are editable display metadata.** Keep them out of curation and label/narration prompts so edits can be folded into `gallery.json` without another AI call. AI must never infer medium. Omit `Artwork.medium` from `gallery.json` and public cards when blank; render `year` independently when supplied. A deliberately cleared artwork title also stays absent from public cards — never synthesize `Untitled` or `Unknown medium` sentinels.
